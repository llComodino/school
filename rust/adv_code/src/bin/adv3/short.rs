...
fn main () {
    ...
    let mut vec0 = vec![0_u32; 5];
    let mut vec1 = vec![0_u32; 5];
    let mut vec2 = vec![0_u32; 5];
    ...
    for (i, c) in lines[max_line].chars().enumerate() {
        line1_dec += c.to_digit(10).unwrap() * 2_u32.pow(4 - i as u32);
    }
    ...
    for (i, c) in lines[max_line].chars().enumerate() {
        line2_dec += c.to_digit(10).unwrap() * 2_u32.pow(4 - i as u32);
    }
