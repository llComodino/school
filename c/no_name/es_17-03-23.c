// Utilizzando un vettore ad una dimensione N = 10 simulare per ogni locazione di memoria un ordine fatto in un negozio di videogame
// Il negozio vendera' soltanto i seguenti articoli
// 1. Gioco PS5 $ 79.99
...
{
    printf("Welcome to RiftStop!\nHere we sell all kinds of overpriced articles!\n\nHave a look around!\n\n");
    for (int i = 0; i < PRD; i++)
    {
...
void print_tot(const double prd_prices[PRD], const char prd_names[][BUF])
{
    ...
    for (int i = PRD; i < SIZE; i++)
    {
        if (prd_prices[i] != 0)
        {
            printf("*%3.0lfx%32s%6.2lf $ *\n", (int)prd_prices[i] / prd_prices[i - PRD], prd_names[i - PRD], prd_prices[i - PRD]);
        }
    }
