...
// - Numero di telefono
// - Indirizzo email
// Terminata la lista trovate il modo per associare ad ogni cliente il proprio
// pacco
...
void initialize_clients(Client *const client) {
  ...
  char *path[ORDERS] = {"clients/client1", "clients/client2", "clients/client3",
                        "clients/client4", "clients/client5"};
  ...
  for (size_t i = 0; i < ORDERS; i++) {
    ...
    if (strcmp(orders[j].address, clients[i].address) == 0) {
      ...
      printf("\nSendind order: %d to %s %s at %s\nSending an email at: "
             "%s\nSending an sms at: %s\n\n",
             orders[j].order_id, clients[i].surname, clients[i].name,
             clients[i].address, clients[i].mail_address, clients[i].phone);
    } else {
      printf("\nError, addresses not matching:\n\nOrder id: %d\nReceiver id: "
             "%d\n\nOrder address: %s\nReceiver address: %s\n\n",
             orders[j].order_id, clients[i].order_id, orders[j].address,
             clients[i].address);
    }
  }
  return;
