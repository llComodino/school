...
bool insert_check(FILE *const check_file, FILE *const id_file,
                  User *const user);
bool verify_check(FILE *const check_file, FILE *const id_file,
                  User *const user);
bool deposit_check(FILE *const check_file, FILE *const accounts_file,
                   User *const user);
...
bool insert_check(FILE *const check_file, FILE *const id_file,
  ...
  while (fread(&compare_check, sizeof(Check), 1, check_file) != 0) {
    ...
    if (strcmp(check.name, compare_check.name) == 0 &&
        strcmp(check.surname, compare_check.surname) == 0) {
